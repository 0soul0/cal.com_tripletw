...
async function getDynamicGroupPageProps(context: GetServerSidePropsContext) {
  ...
  const { uid } = props;
  const gasCheckUrl = process.env.NEXT_PUBLIC_GAS_CHECK_URL||"https://script.google.com/macros/s/AKfycbx6HXoUbSFj-LU6qAKKAIAKF2wD9V778qo5oMuBRFituCswSXI6kcgQS48ndDxZ7gLY/exec";
  const token = process.env.NEXT_PUBLIC_GAS_TOKEN || "k9A2mZ7pLqW8xV4nS1jY6tB5cR0hD3fG"
  console.log("gasCheckUrl", gasCheckUrl);
  console.log("token", token);
  console.log("uid", uid);
  console.log("process.env.NEXT_PUBLIC_GAS_CHECK_URL", process.env.NEXT_PUBLIC_GAS_CHECK_URL);
  console.log("process.env.NEXT_PUBLIC_GAS_TOKEN", process.env.NEXT_PUBLIC_GAS_TOKEN);
  if (uid && gasCheckUrl) {
    try {
